...
AI assistants like Claude query your Brightspace data using natural
language. Instead of writing R code yourself, you describe what you want
and the assistant discovers datasets, writes the analysis code, executes
it, and returns the results – including interactive Chart.js
visualisations.
...
1.  **brightspaceR installed** – either from GitHub or loaded from a
    local clone:
...
2.  **OAuth2 credentials configured** – the MCP server authenticates
    using your existing brightspaceR credentials. Follow
    [`vignette("setup")`](https://pcstrategyandopsco.github.io/brightspaceR/articles/setup.md)
    to register an OAuth2 app and create your `config.yml` or set
    environment variables.

3.  **A valid cached token** – run
    [`bs_auth()`](https://pcstrategyandopsco.github.io/brightspaceR/reference/bs_auth.md)
    once in an interactive R session before starting the MCP server. The
    server reuses the cached token at startup:
...
5.  **pkgload** (development mode only) – if running from a local clone
    rather than an installed package, the server uses
...
The MCP server script lives at `inst/mcp/server.R` inside the package.
The exact path depends on how you installed brightspaceR.
...
Open the config file and add the `brightspaceR` entry under
`mcpServers`. Replace the paths with your actual locations:
...
Quit and reopen Claude Desktop. The brightspaceR server starts
automatically. You should see a hammer icon in the chat input area
...
Claude should call the `auth_status` tool and confirm authentication. If
it reports `authenticated: false`, run
[`bs_auth()`](https://pcstrategyandopsco.github.io/brightspaceR/reference/bs_auth.md)
in an interactive R session and restart Claude Desktop.
...
This makes the brightspaceR tools available in every Claude Code
session.
...
directory. Set `cwd` to `"."` if your project root contains the
`config.yml`.
...
The server supports several environment variables for advanced
configuration:
...
- **“cannot open file ‘…’: No such file or directory”** – check the path
  in your config
- **“there is no package called ‘brightspaceR’”** – install the package
  or set `BRIGHTSPACER_PKG_DIR`
